function roi_svm_perm_withinrun(subject_idx)
    ...
    datadir=[basepath,'/IM2_glmsingle/run-05/'];
    ...
    data=load([datadir,filesep,subjectid,'/TYPED_FITHRF_GLMDENOISE_RR.mat']);
    data=data.modelmd;

    %load in design matrix
    design_file=matchfiles([datadir,filesep,subjectid,'/design_variables*']);
    design=load(design_file{1});
    
    %build condition index from design matrix
    condIndex=design.design;
    condIndex(condIndex(:,2)==1,2)=2;%set entries in column 2 to 2 to identify condition (could be done in a loop if more columns)
    condIndex=sum(condIndex,2);%sum across columns to get a vector (with zeros for every TR)
    condIndex(condIndex==0)=[];%strip out zeros to give final condIndex
    ...
    for mm = 1:numel(masks)
        ...
        maskHDR.fileprefix = [basepath, '/IM2_glmsingle/run-05/constrained_masks/', subjectid, '-', mask_labels{mm}];
    ...
    % Determine the length of the condIndex vector
    condIndexLength = length(condIndex);
    % Create a vector of increasing numbers in pairs
    chunks = repelem(1:condIndexLength/2, 2)';
    ...
    for m = 1:nmasks
        ...
        % Iterate through chunks for partitioning
        for i = 1:numChunks
            % Find the indices of the current chunk for testing
            test_indices = find(chunks == i);
            % Find the indices of the remaining chunks for training
            train_indices = setdiff(1:condIndexLength, test_indices);
            % Store the train_indices and test_indices in cell arrays within the struct
            partitions.train_indices{i} = train_indices;
            partitions.test_indices{i} = test_indices;
        end
    ...
    for permIdx = 1:nPermutations
        ...
        for chunk = 1:max(chunks)
            chunk_indices = find(chunks == chunk); % Find the indices corresponding to the current chunk
            chunk_targets = targets(chunk_indices); % Extract the targets for the current chunk
            shuffled_chunk_targets = chunk_targets(randperm(length(chunk_targets))); % Shuffle the targets within the chunk
            shuffledTargets(chunk_indices) = shuffled_chunk_targets; % Assign the shuffled targets back to the full target vector
        end
    ...
    subject_results_dir = fullfile(datadir, 'svm_results_1', subjectid);
    save_path = fullfile(subject_results_dir, strcat('roi_svm_perm_results_run-05_1_', subjectid, '.mat'));
